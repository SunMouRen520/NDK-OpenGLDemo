...
void MyGLRenderContext::SetImageData(int format, int width, int height, uint8_t *pData) {
    LOGCATE("MyGLRenderContext::SetImageData format = %d, width = %d, height = %d, pData = %d",format, width, height, pData);
...
void MyGLRenderContext::SetImageDataWithIndex(int index, int format, int width, int height,
                                              uint8_t *pData) {
    LOGCATE("MyGLRenderContext::SetImageDataWithIndex index = %d, format = %d, width = %d, height = %d, pData = %d",index, format, width, height, pData);
    NativeImage nativeImage;
    nativeImage.format = format;
    nativeImage.width = width;
    nativeImage.height = height;
...
void MyGLRenderContext::DestoryInstance() {
    LOGCATE("MyGLRenderContext::DestoryInstance");
    if(m_pContext) {
        delete m_pContext;
        m_pContext = nullptr;
    }
