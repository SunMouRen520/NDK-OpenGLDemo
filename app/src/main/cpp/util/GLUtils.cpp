...
GLuint GLUtils::CreateProgram(const char *pVertexShaderSource, const char *pFragShaderSource,
    ...
    GLuint  program = 0;
    FUN_BEGIN_TIME("GLUtils::CrateProgram")
    ...
    if(program) {
        ...
        if(linkStatus != GL_TRUE) {
            ...
            if(bufLength) {
                ...
                if(buf) {
                    glGetShaderInfoLog(program, bufLength, NULL, buf);
    ...
    LOGCATE("GLUtils::CrateProgram program = %d", program);
    return program;
}
...
void GLUtils::CheckGLError(const char *pGLOperation) {
    for(GLint error = glGetError(); error; error = glGetError()) {
        LOGCATE("GLUtils::CheckError GL Operation %s() glError (0x%x)\n", pGLOperation, error);
    }
}
...
/*GLuint GLUtils::CreateProgramWithFeedback(const char *pVertexShaderSource, const char *pFragShaderSource,
    ...
    if(program) {
        ...
        if(linkStatus != GL_TRUE) {
            ...
            if(bufLength) {
                ...
                if(buf) {
                    glGetProgramInfoLog(program, bufLength, NULL, buf);
                    LOGCATE("GLUtils::CrateProgramWithFeedbacks Could not link program: \n%s\n", buf);
                    free(buf);
                }
            }
            glDeleteProgram(program);
    ...
    FUN_END_TIME("GLUtils::CreateProgramWithFeedback")
    LOGCATE("GLUtils::CrateProgramWithFeedbacks program = %d", program);
    return program;
}*/
